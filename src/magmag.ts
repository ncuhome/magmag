...
export class Magmag {
  ...
  private syncStates = (state: StateChanged) => {
    ...
    if (map.size > 1) {
      ...
      if (othersLen > this.playerObjs.length) {
        ...
        if (othersId.length > 0) {
          ...
          const toAdd = othersId.filter(id => !currentIds.includes(id))
    ...
    state.added.forEach(id => {
      ...
      if (otherPlayer?.pos?.x) {
        const newBody = this.createBall(otherPlayer.pos.x, otherPlayer.pos.y, otherPlayer.uid, {
          opacity: 0.6
        })
  ...
  private createBall = (x: number, y: number, id: string, {
    ...
    const attractors = attractorEnabled
      ? [
